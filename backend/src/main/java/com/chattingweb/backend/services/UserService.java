...
public class UserService {
	...
	public MemberResponse findMemberByFullName(String fullName) {
		User u = userRepository.findByFullName(fullName).orElseThrow(() -> new RuntimeException("User not found"));
		return new MemberResponse(u.getId(), u.getFullName(), u.getNickName(), u.getAvatar(), u.getEmail());
	...
	public UserService(UserRepository userRepository, ConversationRepository conversationRepository,
			GroupRepository groupRepository, ConversationMemberRepository conversationMemberRepository) {
		...
		if (group.getOwner().equals(user)) {
			...
			if (!members.isEmpty()) {
				...
				if (newOwnerId != null && members.stream().anyMatch(m -> m.getId().equals(newOwnerId))) {
					...
					// If the new owner ID is not provided or not valid, choose the first member as
					// the new owner
	...
	public User updateUserProfile(UUID userId, String fullName, String nickName, MultipartFile avatar)
			throws IOException {
